#!/usr/bin/env node
/*
 * S3 Player Stats Importer
 *
 * Reads the UBA S3 Player Stats Google Sheet tab via public CSV export
 * and upserts rows into the player_season_stats table.
 *
 * Usage:
 *   SUPABASE_SERVICE_ROLE_KEY=... node scripts/sync-s3-player-stats.mjs
 *
...
const SHEET_ID = '1msTFSlL2F6e4MrFGbkgdwwL8eEhruVx4lR9UGxxvfPg';
...
function parseRows(rawRows) {
  ...
  for (const row of rawRows) {
    ...
    if (/^[A-Z\s]+$/.test(pos) && name === name.toUpperCase() && pos.length > 2) continue; // skip header-like rows

    const team = normalizeTeamCode(row[COL.TEAM]);
    if (!team) continue;

    const gp = parseIntVal(row[COL.GP]);
...
async function main() {
  ...
  console.log(`Fetching S3 player stats from sheet ${SHEET_ID}...`);
